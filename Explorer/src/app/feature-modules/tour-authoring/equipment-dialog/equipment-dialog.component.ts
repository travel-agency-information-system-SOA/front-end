...
console.log('Selected Equipment IDs:', selectedEquipmentIds);
            ...
            this.equipmentService.getEquipment().subscribe((pagedResults: PagedResults<Equipment>) => {
              ...
              this.equipment.forEach(equipment => {
                equipment.selected = selectedEquipmentIds.includes(equipment.id);
  ...
  dodajOpremu() {
    if (this.selectedTour) {
      this.equipment.forEach(item => {
        if (item.selected && item.id !== undefined) {
          if (this.selectedTour.id) {
            this.toureqService.addEquipment(this.selectedTour.id, item.id).subscribe(result => {
              if (result) {
                item.selected = true; 
              }
            });
      ...
      this.equipment.forEach(item => {
        if (!item.selected && item.id !== undefined) {
          if (this.selectedTour.id) {
            this.toureqService.deleteEquipment(this.selectedTour.id, item.id).subscribe(result => {
              if (result) {
                item.selected = false; 
              }
            });
